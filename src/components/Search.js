...
function Search({ isSearch, isFestival, onFilterChange }) {
  ...
  const handleSearch = () => {
    if (search.trim() !== "") {
      const path = isFestival
        ? `/FestivalMap/festival/search/${search}`
        : `/FestivalMap/show/search/${search}`;
  ...
  useEffect(() => {
    setTitle(decodeURIComponent(window.location.pathname.split("search/")[1]));
  }, []);
  return (
    <div>
